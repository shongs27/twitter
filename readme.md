...
비동기를 미들웨어를 통해 쉽게 만드는 것이 redux와 mobx라이브러리들이고 단순히 전역상태관리만을 한다면 contextAPI를 사용하면 된다
...
[리덕스이해하기](https://meetup.toast.com/posts/136)

generator는 중단점이 있어서 함수의 실행이 도중에 멈출 수 있다

thunk와의 차이는 '이펙트'에 있다
throttle, takeLatest, takeLeading같은 이펙트 크리에이터들이 미리 구현되어 있다
...
- 해결책
  ...
  2. 직접 app.use((req,res,next) => res.setHeader()) 미들웨어 처리
...
프론트에서 백엔드로 요청을 보낼때 성공실패 로그 기록 보여주는 라이브러리
...
## 동기적/비동기적 vs 블로킹/논블로킹

- 관점을 어떻게 보느냐의 문제
- 동기적-블로킹 / 비동기적-논블로킹 으로 연결해서 이해하면 된다

[동기/비동기](https://siyoon210.tistory.com/147)

> 동기/비동기는 지시하는 상사, 블로킹/논블로킹은 일하는 사원으로 비유

### 처리작업들이 어떠한 '흐름'으로 실행되냐

동기적 - 작업의 전체'흐름'이 연결되있다
비동기적 - 작업의 전체'흐름'이 뒤죽박죽이다
=> 비동기는 시간 고려하지 않고 작업 여러개 실행함

### 처리작업이 전체 흐름을 막느냐 안막느냐

논블로킹 - 이전 작업이 완료될때까지 대기하지 않고 다음 작업을 수행함
블록킹 - 이전 작업이 끝날때까지 기다렸다가 다음 작업을 수행함
